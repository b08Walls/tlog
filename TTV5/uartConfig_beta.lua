...
function initUART()
    ...
    local function getUartCallBack()
        ...
        local function initMQTT()
            ...
            function sender.imAlive()

                local imAlive = {}
                imAlive.chipid = node.chipid()
                imAlive.ip = wifi.sta.getip()

                local imAliveString = sjson.encode(imAlive)

                print("I AM ALIVE")
                mqttTimer:start()
                sender.m:publish("imAlive",imAliveString,0,0,good)
            end
